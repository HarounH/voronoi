...
class VoronoiEdge:
	...
	def addToPlot(self, boundary):
		...
		if self.ysEnd is None:
			end = self.breakpoint.eval(self.ysStart - boundary)
		else:
			end = self.breakpoint.eval(self.ysEnd)
		# Ensure that the end is in the boundary.
		if ((end.x)>boundary): # Figure out what the end point should be.
			temp = Line(Point(boundary, 0), Point(boundary, 1)).intersection(Segment(start, end))
			if len(temp)>0:
				end = temp[0]
		elif (end.x<(-boundary)):
			temp = Line(Point(-boundary, 0), Point(-boundary, 1)).intersection(Segment(start, end))
		...
		if ((end.y)>boundary): # Figure out what the end point should be.
			temp = Line(Point(0, boundary), Point(-1, boundary)).intersection(Segment(start, end))
			if len(temp)>0:
				end = temp[0]
...
class VoronoiDiagram:
	def __init__(self, lowerBoundary=30):
		self.edges = []
		self.sites = []
